...
function ocaeInit() {
    $.each(ocaRelayCarriers, function (idx, id_carrier) {
        ...
        $(document).ready(function() {
            ...
            if ((!$('#oca-map-'+id_carrier).html()|| !$('#oca-map-'+id_carrier).html().length) && ((idx in windowVisible) && windowVisible[idx]))
                initialize();
            else
                relay_selection($dataContainer.data('ocaBranch') || ocaSelectedRelay);
            if ((!$('#ocaBranchSelect-'+id_carrier+' option') || !$('#ocaBranchSelect-'+id_carrier+' option').length) && (typeof customerAddress !== 'undefined') && (relayAddressIndex in ocaRelays)) {
                loadRelayList();
            }
            carrier_selection();
        });
        ...
        function changeState(e, params) {
            ...
            if (!params.load && ocaRelays[relayAddressIndex][$dataContainer.data('ocaBranch')] && translateStateName(ocaRelays[relayAddressIndex][$dataContainer.data('ocaBranch')].Provincia) !== params.selected.toLowerCase())
        ...
        function assignBranch(index, previous) {
            ...
            var bounds = new google.maps.LatLngBounds();
            if ((typeof home !== 'undefined') && home.lat && home.lng && map) {
                bounds.extend(new google.maps.LatLng(home.lat, home.lng));
